  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        sendProgress(writer, "Fetching signals...", 0);
        ...
        sendProgress(writer, `Loaded ${dataPoints.length} signals`, 0);
        ...
        sendProgress(writer, "Running Layer 1: Primary Analysis...", 1);
        ...
        if (layer1Themes && layer1Themes.length > 0) {
          sendProgress(writer, `Layer 1 complete. Found ${layer1Themes.length} themes. Running Layer 2...`, 1);
          sendProgress(writer, "Running Layer 2: Deep Review & Audit...", 2);
        ...
        sendProgress(writer, "Saving themes to database...", 2);
        ...
        sendProgress(writer, `Analysis complete! Generated ${processedThemeIds.length} themes.`, 2);
        sendProgress(writer, "Done!", 2);

        writer.write(`data: ${JSON.stringify({ themes: processedThemeIds.length, layers: 2 })}\n\n`);
