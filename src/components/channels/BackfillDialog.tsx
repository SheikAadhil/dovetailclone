...
export function BackfillDialog({ channelId, isOpen, onClose, onSuccess }: BackfillDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="rounded-[2.5rem] p-0 border-none shadow-2xl max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-10 pt-4 border-t border-gray-50 bg-gray-50/30">
          <DialogFooter className="gap-3 sm:justify-start">
            <Button variant="outline" onClick={onClose} disabled={loading} className="rounded-2xl h-14 px-8 font-black text-xs uppercase tracking-widest border-gray-200 bg-white">
              Cancel
            </Button>
